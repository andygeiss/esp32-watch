...
firmware fills it from the hardware, and `ui.c` never learns which.
...
`lvgl/` is gitignored, so a fresh clone needs that one command. `make` on its
own is `make check`: the `lv_conf.h` liveness grep, the build, the boundary
check, and 74 assertions rendered into a byte array by `test_ui.c` — geometry,
