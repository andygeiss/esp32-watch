...
| | |
|---|---|
| `ui.c`, `ui.h`, `ui_font_*.c` | **portable.** LVGL and the C standard library, nothing else. Both builds compile these unchanged |
| `main.c` | **host only.** SDL window, mouse, tick source, service loop |
| `voice.c` | **host only.** The microphone, the two speech services, and the speaker |
...
`ui.c` alone and fails if `nm` finds anything undefined but `lv_*` and libc. In
...
| | |
|---|---|
| `ui.c`, `ui.h` | the clock face, the assistant face, and the morph between them |
| `ui_font_digits_118.c`, `ui_font_date_72.c`, `ui_font_assistant_18.c` | generated by `tools/gen_fonts.py`; digits with tabular figures, and the two FontAwesome glyphs LVGL does not ship |
| `main.c`, `CMakeLists.txt` | the simulator |
| `voice.c`, `voice.h` | the voice loop: microphone, Parakeet, Chatterbox, speaker |
| `test_ui.c` | the headless renderer that `make test` runs |
