...
| `voice/` | **portable, stricter.** `turn.c` is everything about a turn that is not a device — the words, the gate, the wire. The C standard library and *nothing else*, not even LVGL |
| `host/` | **host only.** `main.c` for the window, mouse, tick source and service loop; `voice.c` for the SDL microphone, the socket and the SDL speaker; `test_ui.c` for the headless renderer |
...
| `voice/turn.c`, `voice/turn.h` | the portable half of the voice loop: the words, the gate, the JSON, the WAV, both request bodies |
| `voice/voice.h` | the five functions each platform's voice loop implements |
| `host/main.c`, `CMakeLists.txt` | the simulator |
| `host/voice.c` | its voice loop: SDL microphone, a socket, SDL speaker |
