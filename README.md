...
the timezone are `idf.py -C firmware menuconfig`, under **ESP32 Watch**. The board's RTC sets the clock at boot once SNTP has set it once, so without
an SSID a watch that has never seen a network counts from reset and the WiFi corner draws dim, which is the reading that dimming
